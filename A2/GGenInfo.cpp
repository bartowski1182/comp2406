...
void GGenInfo::drawGenInfo(int selection)
{
  ...
  mvprintw(LINES - 4, 0, "This is your selection %d\n", selection);
...
int GGenInfo::handleSelection(int selection)
{
  ...
  else if(index > 0 && index < 8)
  {
    ...
    if(index < 5)
      ...
      textFields[index - 2].assign(sInput);
  }
  else
  {
    if(index == 8) { cancel = true; return nBreaker = index; }
    if(index == 9) {  handleAccept(); }
  }
...
bool GGenInfo::initGenInfo()
{
  ...
  loadResearch = new CourseLoader("researchAreas.txt");
  int a = loadResearch -> loadCourses(&researchAreaList);
  delete loadResearch;
