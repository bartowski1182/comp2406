...
TCourses::TCourses(std::string s, TaCourseQueue **aq) : index(1), course(s), nBreaker(0), relatedCourse(s), courseIndex(1), tAHead(aq), cancel(false)
{
  labels[0] = "Term:";
  labels[1] = "Year:";
  labels[2] = "Supervisor:";
...
////////////////////////////////////////////////////////////////////////////
void TCourses::drawTCourses(int selection)
{
  char ccourse[TCMAX_BUF + 1];
  for(int i = 0; i < TCMAX_BUF; i++)
  {
    if(i < course.length())
      ccourse[i] = course[i];
    else
  ...
  for(i = 0; i < TCNUM_CHOI; i++) //Draw the labels for the text fields
  {
    for(int j = 0; j < TCMAX_BUF; j++)
    {
  ...
  for(i = 0; i < TCNUM_CHOI - 3; i++) //Draw the textFields
  {
...
////////////////////////////////////////////////////////////////////
int TCourses::drawCourseView(int selection)
{
  int x, y, i;
  char c[TCMAX_BUF + 1];
...
////////////////////////////////////////////////////////////////////
int TCourses::handleInput(char c)
{
...
//          value  </summary>                            //
///////////////////////////////////////////////////////////
int TCourses::handleSelection(int selection)
{
  if(index == 0) initCourseView(selection);
  if(index > 0 && index < 4)
  {
    char cInput[TCMAX_BUF], c[TCMAX_BUF + 1];
...
int TCourses::handleAdd()
{
  ...
  {
    if((*tAHead) == 0)
      (*tAHead) = new TaCourseQueue;
    
    (*tAHead) -> pushBack(newTAC);
  }
...
TaCourse* TCourses::checkValid()
{
  ...
  if(testPassed)
  {
    newTAC = new TaCourse(relatedCourse, textFields[0], textFields[1], textFields[2]);
...
////////////////////////////////////////////////////////////////////
int TCourses::initCourseView(int prevIndex)
{
  clear();
...
bool TCourses::initTCourses()
{
  ...
  while(1)
  {
    ...
    drawTCourses(index); //Redraw the UI based on any changes made by the user
